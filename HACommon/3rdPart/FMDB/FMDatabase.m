...
-(BOOL)tableAndColNameIsExist:(NSString*)tbName colName:(NSString*)colName
{
    BOOL ret = NO;
    NSString* sqlString = [NSString stringWithFormat:@"select sql from sqlite_master where tbl_name='%@'", tbName];
    sqlite3_stmt *statement = nil;
    if (sqlite3_prepare_v2(_db, [sqlString UTF8String], -1, &statement, NULL) != SQLITE_OK) {
        NSLog(@"Error:select failed to prepare statement with message:%d.%s",sqlite3_errcode(_db),sqlite3_errmsg(_db));
		return NO;
    }
	
	if (sqlite3_step(statement) == SQLITE_ROW)
	{
		NSString *strText = [NSString stringWithFormat:@"%s",sqlite3_column_text(statement,0)];
		if ( [strText rangeOfString: colName].location != NSNotFound)
			ret = YES;
	}
...
-(BOOL)tableIsExist:(NSString*)tbName
{
    BOOL ret = NO;
    NSString* sqlString = [NSString stringWithFormat:@"select count(*) from sqlite_master where type='table' and name='%@'", tbName];
    sqlite3_stmt *statement = nil;
    if (sqlite3_prepare_v2(_db, [sqlString UTF8String], -1, &statement, NULL) != SQLITE_OK) {
        NSLog(@"Error:select failed to prepare statement with message:%d.%s",sqlite3_errcode(_db),sqlite3_errmsg(_db));
		return NO;
	...
	{
		NSString *strText = [NSString stringWithFormat:@"%s",sqlite3_column_text(statement,0)];
		if ([strText isEqualToString:@"0"] == NO)
			ret = YES;
	}
